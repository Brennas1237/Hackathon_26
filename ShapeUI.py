...
class ShapeUI:
    ...
    def _build_controls(self):
        ...
        self.sink_material_var.trace_add("write", self._sink_selection_changed)
        self.surround_material_var.trace_add("write", self._surround_selection_changed)
    ...
    def _sink_selection_changed(self, *args):
        if self.sink_material_var.get() == "Custom":
            if self.sink_custom_entry.get() == "---":
                self.sink_custom_entry.delete(0, "end")  # clear placeholder
        else:
            self.sink_custom_entry.delete(0, "end")
            self.sink_custom_entry.insert(0, "---")
            self.sink_custom_entry.config(bg="white")
            self.sink_error_label.config(text="")

    def _surround_selection_changed(self, *args):
        if self.surround_material_var.get() == "Custom":
            if self.surround_custom_entry.get() == "---":
                self.surround_custom_entry.delete(0, "end")  # clear placeholder
        else:
            self.surround_custom_entry.delete(0, "end")
            self.surround_custom_entry.insert(0, "---")
            self.surround_custom_entry.config(bg="white")
            self.surround_error_label.config(text="")
    ...
    def integrate_shape(self):
        ...
        sink_k = Sink_Materials.get(self.sink_material_var.get())
        if self.sink_material_var.get() == "Custom":
            try:
    ...
    def run_physics(self):
        ...
        # math.update_heats(drawn_points, heat_source_temp, ambient_temp, sink_k, surround_h)
...
class ShapeUI:
    ...
    def integrate_shape(self):
        ...
        if self.sink_material_var.get() == "Custom":
            try:
                val = float(self.sink_custom_entry.get())
                if val <= 0:
                    raise ValueError
                sink_k = val
                self.sink_custom_entry.config(bg="white")
                ...
                self.sink_custom_entry.config(bg="#ff9999")  # red background
                self.sink_error_label.config(text="Must be a positive number")
                return
        else:
            sink_k = Sink_Materials.get(self.sink_material_var.get())
            self.sink_custom_entry.config(bg="white")
        ...
        if self.surround_material_var.get() == "Custom":
            try:
                val = float(self.surround_custom_entry.get())
                if val <= 0:
                    raise ValueError
                surround_h = val
                self.surround_custom_entry.config(bg="white")
                ...
                self.surround_custom_entry.config(bg="#ff9999")  # red background
                self.surround_error_label.config(text="Must be a positive number")
                return
        else:
            surround_h = Surrounding_Materials.get(self.surround_material_var.get())
            self.surround_custom_entry.config(bg="white")
...
class ShapeUI:
    ...
    def _build_controls(self):
        ...
        tk.Label(self.control_frame, text="Custom k (Sink)", font=("Arial", 10)).pack(anchor="w")
        self.sink_custom_entry = tk.Entry(self.control_frame)
        self.sink_custom_entry.insert(0, "---")  # default placeholder
        ...
        tk.Label(self.control_frame, text="Custom h (Surround)", font=("Arial", 10)).pack(anchor="w")
        self.surround_custom_entry = tk.Entry(self.control_frame)
        self.surround_custom_entry.insert(0, "---")  # default placeholder
